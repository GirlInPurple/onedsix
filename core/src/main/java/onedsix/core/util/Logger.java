...
public class Logger {
    ...
    public void info(String text) {
        String[] printed = {generateTag("Info "), text};
        if (!Vars.envType.isDedicated) Gdx.app.log(printed[0], printed[1]);
        else System.out.println("["+printed[0]+"] "+printed[1]);
        WRITER.print("["+printed[0]+"] "+printed[1]+"\n");
    }
    
    /** Standard Error Logger */
    public void error(String text) {
        String[] printed = {generateTag("Error"), text};
        if (!Vars.envType.isDedicated) Gdx.app.error(printed[0], printed[1]);
        else System.out.println("["+printed[0]+"] "+printed[1]);
        WRITER.print("["+printed[0]+"] "+printed[1]+"\n");
    }
    ...
    public void error(String text, Throwable e) {
        String[] printed = {generateTag("Error"), text};
        if (!Vars.envType.isDedicated) Gdx.app.error(printed[0], printed[1], e);
        else System.out.println("["+printed[0]+"] "+printed[1]);
        WRITER.print("["+printed[0]+"] "+printed[1]+"\n"+e+"\n");
    }
    ...
    public void debug(String text) {
        String[] printed = {generateTag("Debug"), text};
        if (!Vars.envType.isDedicated) Gdx.app.debug(printed[0], printed[1]);
        else System.out.println("["+printed[0]+"] "+printed[1]);
        WRITER.print("["+printed[0]+"] "+printed[1]+"\n");
    }
